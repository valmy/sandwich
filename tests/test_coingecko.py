...
class TestSaveMarketData:
    ...
    def test_multiple_pages_fetch(self, tmp_path, capsys):
        from sandwich.coingecko.markets import save_market_data

        page1_data = get_coingecko_page_data(1)
        page2_data = get_coingecko_page_data(2)
        ...
        with patch("sandwich.coingecko.markets.make_request") as mock_make_request:
            ...
            captured = capsys.readouterr()
            assert "Number of items in the list: 5" in captured.out

            with open(file_path, "r") as f:
                data = json.load(f)
                assert len(data) == 5
                assert data[0]["symbol"] == "btc"
                assert data[3]["symbol"] == "sol"

    def test_empty_response(self, tmp_path, capsys):
        from sandwich.coingecko.markets import save_market_data
        ...
        with patch(
            ...
            captured = capsys.readouterr()
            assert "Number of items in the list: 0" in captured.out

            with open(file_path, "r") as f:
                data = json.load(f)
                assert len(data) == 0

    def test_malformed_json_response(self, tmp_path, capsys):
        from sandwich.coingecko.markets import save_market_data
    ...
    def test_network_error_recovery(self, tmp_path):
        ...
        with patch("sandwich.coingecko.markets.make_request") as mock_make:
            mock_make.side_effect = [mock_success, mock_success]
            save_market_data(str(file_path))
